...
// allow CORS (set your frontend origin in Render env vars)
  ...
  return new Promise((resolve, reject) => {
    ...
    const mailLine = (label, arr) => {
      ...
      arr.forEach((addr) => {
        doc
          .fillColor("#1d4ed8")
          .text(addr, { link: `mailto:${addr}`, underline: true })
          .fillColor("black");
      });
      doc.moveDown(0.3);
    };
    ...
    if (sigBuf) {
      ...
      try {
        doc.image(sigBuf, x, y, { fit: [sigW, sigH], align: "left", valign: "center" });
      } catch (e) {
        console.error("Signature render failed:", e.message);
      }
      doc.moveDown(2);
    }
    ...
    if (safePhotos.length > 0) {
      ...
      for (const p of safePhotos) {
        ...
        doc.save();
        doc.rect(x - 1, rowY - 1, cellW + 2, cellH + 2).stroke("#e5e7eb");
        doc.restore();

        if (b) {
          try {
            doc.image(b, x, rowY, { fit: [cellW, cellH], align: "center", valign: "center" });
          } catch (e) {
            console.error("Photo render failed:", e.message);
          }
        }
        ...
        doc
          .fontSize(9)
          .fillColor("#555")
          .text(caption, x, rowY + cellH + 2, { width: cellW, align: "center" })
          .fillColor("black");
...
app.post("/api/signoff", async (req, res) => {
  try {
    ...
    if (!to.length) {
      return res.status(400).json({ ok: false, error: "No recipient (toEmail or TO_EMAIL)." });
    }

    const subject =
      body.subject?.trim() || `OSD Sign-Off${body.poNumber ? ` – ${body.poNumber}` : ""}`;
